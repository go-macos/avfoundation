...
`OpenPlayer` both report `ErrNoVideoTrack` for them. MP4, MOV and M4V work.
That path exists elsewhere: demux with
[`go-avkit/avkit/container`](https://github.com/go-avkit/avkit), which does read
EBML, and decode with
[`go-macos/videotoolbox`](https://github.com/go-macos/videotoolbox), which feeds
the elementary stream to VideoToolbox directly and hands back the same zero-copy
BGRA `Frame` this package does. Its `cmd/vtprobe` decodes a 1 h 32 MKV end to
end, and its first ten frames of a control MP4 are byte-for-byte what `avprobe`
produces here.
